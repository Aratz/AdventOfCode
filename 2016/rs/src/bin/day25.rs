mod day12 {
    ...
    impl Instruction {
        fn new(s: &str) -> Self {
            ...
            match words[0] {
                ...
                "out" => Instruction::Out(Variable::new(words[1])),
    ...
    fn execute(instructions: &[Instruction], mut regs: [i32; NREG]) -> Vec<i32> {
        let mut instructions = instructions.to_vec();
        let mut i = 0;

        let mut transmitted = Vec::new();

        while i < instructions.len() && transmitted.len() < MAXLEN {
    ...
    fn is_clock(signal: &[i32]) -> bool {
        signal.iter().enumerate().all(|(i, x)| x + ((i as i32 + 1)%2) == 1)
    }

    pub fn solve(lines: &[String]) -> i32 {
        let instructions = lines.iter()
            .map(|s| Instruction::new(s)).collect::<Vec<_>>();

        (0..).find(|&a| is_clock(&execute(&instructions, [a, 0, 0, 0]))).unwrap()
    }
}
...
fn main() {
    ...
    println!("Solution A-part: {}", day12::solve(&lines));
}
