...
fn main() {
    ...
    computer.compute();
    loop {
        let x = match computer.get_next_output() {
            Some(x) => x,
            None => {println!("No output"); break},
        };

        let y = computer.get_next_output().unwrap();
        let tile_id = computer.get_next_output().unwrap();

        screen.insert((x, y), tile_id);
    }

    println!("len {}", screen.len());
    println!("Block tiles: {}", screen.values().filter(|&tile_id| *tile_id==2).count());
}
