...
fn main() {
    ...
    loop {
        ...
        let new_moves = legal_moves.iter()
            .filter(|m| map.get(&m.1).is_none())
            .collect::<Vec<_>>();

        let possible_moves = legal_moves.iter().filter(|m| match map.get(&m.1) {
                Some(-1) => false,
                _ => true,
            }).collect::<Vec<_>>();

        let command = match rng.choose(&new_moves) {
            Some((c, _)) => *c,
            None => rng.choose(&possible_moves).unwrap().0,
        };
        ...
        pos = match computer.get_next_output() {
            Some(0) => { map.insert(attempted_move, -1); pos },
            Some(1) => { 
                if found && !map.contains_key(&attempted_move) {
                    let curr_dist = map[&pos];
                    map.insert(attempted_move, curr_dist + 1);
                };
                attempted_move
            },
            Some(2) => {
                if !found {
                    map.insert(attempted_move, 0);
                    found = true;
                    println!("FOUND! Starting measuring distances");
                }
                attempted_move
            },
        ...
        if i % 1000 == 0 {
            if !found {
                println!("Searching (i:{}) ...", i);
            }
            else {
                println!("Analysing terrain: max={}", map.values().max().unwrap());
            }
        }
