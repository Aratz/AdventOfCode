...
fn main() {
    ...
    let mut nics:Vec<AmpComputer> = (0..50).map(|_| AmpComputer {
                i: 0,
    ...
    let mut packet_assembly: Vec<(Option<i64>, Option<i64>)> =
        vec![(None, None); 50];
    //several packages may assemble a packet for the same address at the same time

    while !packet_queue.contains_key(&255) {
        for (i, nic) in nics.iter_mut().enumerate(){
            //Add input if any
            ...
            if let Some(output) = nic.get_next_output() {
                match packet_assembly[i] {
                    ...
                    (Some(add), Some(x)) => {
                        packet_queue.entry(add as usize).or_default().push_back((x, output));
    ...
    println!("{}", packet_queue[&255].front().unwrap().1);
}
